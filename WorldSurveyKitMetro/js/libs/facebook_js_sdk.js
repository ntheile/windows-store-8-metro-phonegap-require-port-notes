  ...
  ui: function(params, cb) {
    ...
    if (FB._nativeInterface) {
        switch (params.method) {
            case 'auth.login':
                FB._nativeInterface.login(params, cb, function(e) {alert('Cordova Facebook Connect plugin fail on auth.login login!' + e);});
                break;
            case 'permissions.request':
                FB._nativeInterface.login(params, cb, function (e) { alert('Cordova Facebook Connect plugin fail on  permissions.request login!' + e); });
                break;
            case 'permissions.oauth':
                FB._nativeInterface.login(params, cb, function (e) {
                    alert('Cordova Facebook Connect plugin fail on permissions.oauth login!' + e);

                    try {
                        App.authView.fbCancel();
                    }
                    catch (err) {}
