...
var TDFriendSelector = (function (module, $) {
    ...
    buildFriendSelector = function (callback) {
        ...
        // Check that the user is logged in to Facebook
        $.get('https://graph.facebook.com/me/friends?fields=id,name&access_token=' + App.user.get("access_token"), function (response) {
            if (response.data) {
                setFriends(response.data);
                // Build the markup
                buildMarkup();
        ...
        buildFriendMarkup = function (friend) {
            return '<a href="#" class="TDFriendSelector_friend TDFriendSelector_clearfix" data-id="' + friend.id + '" data-name="' + friend.name + '" >' +
					'<img src="https://graph.facebook.com/' + friend.id + '/picture?type=square" width="50" height="50" alt="' + friend.name + '" class="TDFriendSelector_friendAvatar" />' +
