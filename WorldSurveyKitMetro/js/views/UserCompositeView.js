...
define(["jquery", "backbone", "models/Models", "views/UserItemView", "tdfriendselector"],
    ...
    var UserCompositeView = Backbone.View.extend({
        ...
        addFBUserById: function () {
            ...
            $.get('https://graph.facebook.com/' + fid + "?access_token=" + App.user.get("access_token"), function (response) {
                var name = toStaticHTML(response.name);
                var ary = [];
                ary.push({
                    fid: fid,
                    name: name
